...
def send_commands_not_supported(message, content_types = ['text']):
...
def parse_and_send_messages(message):
    bot.send_message(message.chat.id, f'''Постинг запущен. Для обработки потребуется немного времени. Если изображения не получены проверьте настройки бота группы/канала.''')
    ...
    try:
        ...
        bot.send_message(message.chat.id, f'''Произошла ошибка при отправки ссылки.''')
...
def start_posting_handler(message):
    ...
    scheduler.start()
    bot.send_message(message.chat.id, f'Планировщик запущен')
